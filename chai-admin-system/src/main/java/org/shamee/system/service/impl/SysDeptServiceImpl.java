    ...
    implements SysDeptService
{
    ...
    public List<SysDeptTreeResp> getDeptTree() {
        List<SysDept> listed = this.lambdaQuery()
            ...
            .eq(SysDept::getStatus, CommonConstant.Status.ENABLED)
            .list();
    ...
    @Override
    public PageResult<SysDeptPageResp> getDeptPage(
        PageRequest<SysDeptQueryRequest> pageRequest
    ) {
        Page<SysDeptPageResp> deptPageResps = baseMapper.selectDeptList(
            pageRequest.page(),
    ...
    public List<SysDept> getChildDepts(String parentId) {
        return baseMapper.findChildDepts(parentId);
    }
    ...
    public boolean deleteDept(String deptId) {
        ...
        // 检查是否有角色关联此部门
        if (
            sysRoleDeptService.count(
                Wrappers.lambdaQuery(SysRoleDept.class).eq(
    ...
    @Override
    public boolean checkDeptNameExists(
        String deptName,
        String parentId,
        String excludeId
    ) {
    ...
    private List<SysDeptTreeResp> buildDeptTreeResult(List<SysDept> depts) {
        ...
        for (SysDept dept : depts) {
            if (
                ...
                dept.getParentId().equalsIgnoreCase("0")
            ) {
                // 根部门
     ...
     * 递归获取子部门
     */
    private List<SysDept> getChildDepts(
        String parentId,
        List<SysDept> allDepts
    ) {
        List<SysDept> childDepts = allDepts
            .stream()
    ...
    public List<String> getDeptAndChildIds(String deptId) {
        return baseMapper.getDeptAndChildIds(deptId);
    }
}
