...
import org.shamee.common.util.copy.BeanCopierCallBack;
...
public class PageResult<T> extends PageInfo {
    ...
    public static <T> PageResult<?> of(int total, int pageSize, List<?> records) {
        return new PageResult<T>(total, pageSize, records);
    }
    ...
    public static <S, T> PageResult<T> of(Page<S> page, Supplier<T> supplier, BeanCopierCallBack<S, T> callBack) {
        List<T> objects = BeanCopierUtils.copyListProperties(page.getRecords(), supplier, callBack);
        return new PageResult<T>((int) page.getTotal(), (int) page.getSize(), objects);
    }
